...
async def list_services(
    ...
    total_q = await db.execute(
        select(func.count()).select_from(Service).where(Service.user_id == user.id)
    )
...
async def get_service(
    service_id: uuid.UUID, db: DBSession, user: CurrentUser
) -> ServiceResponse:
...
async def create_alert_rule(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Alert rule already exists"
        )
    return rule
...
async def delete_alert_rule(
    service_id: uuid.UUID, rule_id: uuid.UUID, db: DBSession, user: CurrentUser
) -> None:
    await _get_service_or_404(db, service_id, user.id)
    result = await db.execute(
        select(AlertRule).where(AlertRule.id == rule_id, AlertRule.service_id == service_id)
    )
