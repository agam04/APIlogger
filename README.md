...
**[Live Demo → ap-ilogger.vercel.app](https://ap-ilogger.vercel.app/)**
...
- **Multi-node probing** — 2 checker nodes independently probe every registered service on a configurable interval
- **Quorum-based incident detection** — ≥51% of nodes must agree a service is down before an incident opens, eliminating single-node false alerts
- **At-least-once delivery** — Redis Streams XREADGROUP + XAUTOCLAIM guarantees no check result is lost even if a node crashes mid-task
- **AI incident reports** — structured root cause, confidence score, risk level, and fix steps generated by Groq Llama-3.3-70b or Anthropic Claude
- **Real-time dashboard** — Server-Sent Events push status changes to the browser instantly without polling
- **p50/p95/p99 latency tracking** — computed server-side via PostgreSQL native `percentile_cont`
- **Alert rules** — email and Slack/Discord webhook notifications per service, configurable per incident open/resolve
- **Full auth** — JWT-based, per-user service isolation
...
- (Optional) `GROQ_API_KEY` for AI summaries — free at [console.groq.com](https://console.groq.com)
- (Optional) `ANTHROPIC_API_KEY` — takes priority over Groq if both are set
...
|---|---|---|
| Frontend | Vercel | Static React build, set `VITE_API_URL` to coordinator URL |
| Coordinator | Railway | `coordinator/Dockerfile`, runs `alembic upgrade head` on start |
| Checker nodes | Railway | `checker/Dockerfile`, set `NODE_ID` differently per instance |
| PostgreSQL | Neon / Railway plugin | Use `postgresql+asyncpg://` scheme |
| Redis | Railway plugin | Reference as `${{Redis.REDIS_URL}}` |
